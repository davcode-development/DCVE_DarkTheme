...
def checkConn():
    try:
        ...
        cprint(colors.FAIL, 'An Internet connection is required to install DCVE_DarkTheme.', True)
...
def installTheme():
    clear()
    doHeader()
    baseURL = os.getenv('BASEURL', 'https://github.com/davcode-development/DCVE_DarkTheme/raw/' + os.getenv("BRANCH", "master"))
    ...
    urllib.request.urlretrieve(baseURL + '/DCVE_DarkTheme/sass/DCVE_DarkTheme.css', '/usr/share/pve-manager/css/dd_style.css')
    cprint(colors.NORMAL, 'Downloading patcher..')
    urllib.request.urlretrieve(baseURL + '/DCVE_DarkTheme/js/DCVE_DarkTheme.js', '/usr/share/pve-manager/js/dd_patcher.js')
    ...
    for index, image in enumerate(images):
        ...
        urllib.request.urlretrieve(baseURL + '/DCVE_DarkTheme/images/' + image, '/usr/share/pve-manager/images/' + image)
...
def doHeader():
    ...
    cprint(colors.NORMAL, ' DCVE_DarkTheme Utility\n', False, True)
...
def main():
    parser = argparse.ArgumentParser(description='DCVE_DarkTheme Theme Utility')
